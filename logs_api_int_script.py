...
def create_tmp_table_for_insert(db, table, date1, date2):
    q = '''
        ...
        SELECT
            ...
            OSName
        FROM tmp_data
        WHERE NOT ((EventDate, DeviceID, EventName, EventTimestamp, AppPlatform, Country, APIKey, AppVersionName, AppBuildNumber, AppPackageName, City, OSName) GLOBAL IN
            (SELECT
                EventDate,
                DeviceID,
                EventName,
    ...
    for df in pd.read_csv(StringIO(r.content.decode('utf-8')), chunksize=PROCESSING_ROWS, iterator=True):
        ...
        upload(
            ...
            df[['event_date', 'appmetrica_device_id', 'event_name', 'event_timestamp',
                'os_name', 'country_iso_code', 'api_key', 'app_version_name', 'app_build_number',
                'app_package_name', 'city', 'os_name']].to_csv(index=False, sep='\t')
        )
